...
class MachineName extends Textfield {
  ...
  public static function processMachineName(&$element, FormStateInterface $form_state, &$complete_form) {
    ...
    // Handle case where machine name uses hyphens rather than underscores.
    if (array_key_exists('#machine_name', $element) &&
      !empty($element['#machine_name']['replace']) &&
      $element['#machine_name']['replace'] == '-') {
      $replace_character_name = 'hyphens';
    }
    else {
      $replace_character_name = 'underscores';
    }

    // Apply default form element properties.
    $element += [
      '#title' => t('Machine-readable name'),
      '#description' => t('A unique machine-readable name. Can only contain lowercase letters, numbers, and @replace_character_name .', ['@replace_character_name' => $replace_character_name]),
      '#machine_name' => [],
      '#field_prefix' => '',
      '#field_suffix' => '',
      '#suffix' => '',
